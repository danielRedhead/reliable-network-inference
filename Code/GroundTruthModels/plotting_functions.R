...
parameter_recovery_decay_plots <- function(PP, fit, Input,  decay_curve, ID){
 recov_res_latent_m <- recov_res_latent_l <- recov_res_latent_h <- matrix(NA, nrow=PP, ncol=length(decay_curve))

 Input=round(Input,4)
 
# First extract sample from Stan    
for(k in 1:PP){
 decay_curve_samp = extract(fit[[k]],pars="decay_curve")$decay_curve

# Now organize estimates of interest
                                              
  x <- apply(decay_curve_samp, 2, HPDI)

  recov_res_latent_m[k,] <- rev(apply(decay_curve_samp, 2, mean))
  recov_res_latent_l[k,] <- rev(x[1,])
  recov_res_latent_h[k,] <- rev(x[2,])
 }

 Estimate  = c(1:length(decay_curve))

 mp <- vector("list",length(decay_curve))
 for(k in 1:length(decay_curve))
 mp[[k]] <- data.frame(Value=c(rep(decay_curve[k],PP), recov_res_latent_m[,k]), 
                  L=c(rep(NA,PP),recov_res_latent_l[,k]),
                  H=c(rep(NA,PP),recov_res_latent_h[,k]), 
                  Input=rep(Input,2),
                  Mode=rep( c("True","Estimated"), each=PP),
...
if(ID=="MIX" | ID == "FPR"){
  title= "False positive rate displayed in facet caption"
}
if(ID=="FPV"){
  title= "Dispersion in false positive rate displayed in facet caption"
}
if(ID=="RTT"){
  title= "True tie recall rate displayed in facet caption"
}
if(ID=="RTV"){
  title= "Dispersion in true tie recall rate displayed in facet caption"
}
if(ID=="TV"){
  title= "Dispersion in theta rate displayed in facet caption"
}
if(ID=="TR"){
  title= "Theta rate displayed in facet caption"
}

###############
(p4 <- ggplot(mp, aes(x=Period, y=Value, color=Mode, fill=Mode))+
    geom_line(size=1) +
    geom_ribbon(aes(ymin=L, ymax=H), alpha=0.3, colour = NA) +
    facet_wrap(. ~ Input, scales = "fixed", nrow=5) +
    scale_fill_manual(name = "Mode:",values = cols) +
    scale_colour_manual(name = "Mode:",values = cols) + xlab("Time-step lags") + ylab("Log-odds offset for true tie recall rate") +
        theme(legend.title=element_text(size=14), 
          legend.text=element_text(size=14))+
    ...
    theme(axis.text=element_text(size=12),
        axis.title=element_text(size=16))+
    theme(panel.background = element_rect(fill = "gray91", colour = "gray91", size = 0.5, linetype = "solid")) +
     theme(plot.margin = unit(c(0.1,0.4,0.1,0.1), "cm"))+ theme(legend.position="bottom") + ggtitle(title) +
    scale_x_continuous(breaks=c(1,5,10,15,20))
    )

ggsave(paste0("ParamRecov_decay_",ID,".pdf"), p4, height=8, width=8)
}



parameter_recovery_flow_plots <- function(PP, fit, Input,  flow_rate, ID){
 recov_res_latent_m <- recov_res_latent_l <- recov_res_latent_h <- matrix(NA, nrow=PP, ncol=length(flow_rate))

   Input=round(Input,4)
 
# First extract sample from Stan    
for(k in 1:PP){
 flow_rate_samp = extract(fit[[k]],pars="flow_rate")$flow_rate

# Now organize estimates of interest
                                              
  x <- apply(flow_rate_samp, 2, HPDI)

  recov_res_latent_m[k,] <- apply(flow_rate_samp, 2, mean)
  recov_res_latent_l[k,] <- x[1,]
  recov_res_latent_h[k,] <- x[2,]
 }

 Estimate  = c(1:length(flow_rate))

 mp <- vector("list",length(flow_rate))
 for(k in 1:length(flow_rate))
 mp[[k]] <- data.frame(Value=c(recov_res_latent_m[,k], rep(flow_rate[k],PP)), 
                  L=c(recov_res_latent_l[,k],rep(NA,PP)),
                  H=c(recov_res_latent_h[,k],rep(NA,PP)), 
                  Input=rep(Input,2),
                  Mode=rep( c("Estimated","True"), each=PP),
                  Period=Estimate[k]
                  )
...
###############
(p4 <- ggplot(mp, aes(x=Period, y=Value, color=Mode, fill=Mode))+
    geom_linerange(aes(ymin=L, ymax=H, colour = Mode), alpha=0.7, size=2) +
        geom_point(size=1) +
    facet_wrap(. ~ Input, scales = "fixed", nrow=5) +
    scale_fill_manual(name = "Mode:",values = cols) +
    scale_colour_manual(name = "Mode:",values = cols) + xlab("Time-step") + ylab("Flow rate of goods over true ties") +
        theme(legend.title=element_text(size=14), 
          legend.text=element_text(size=14))+
    theme(strip.text.x = element_text(size = 10),
      title = element_text(size = 14))+
...
ggsave(paste0("ParamRecov_flow_",ID,".pdf"), p4, height=8, width=8)
}
