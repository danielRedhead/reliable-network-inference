...
recov_res_true_m[,16] <- rep(theta_mean_fpv, PP)
recov_res_true_m[,17] <- rep(theta_sigma_fpv, PP)
...
recov_slope_true_m[,1] <- rep(fpr_effects_1_fpv[1], PP)
recov_slope_true_m[,2] <- rep(fpr_effects_1_fpv[2], PP)
recov_slope_true_m[,3] <- rep(rtt_effects_1_fpv[1], PP)
recov_slope_true_m[,4] <- rep(rtt_effects_1_fpv[2], PP)
recov_slope_true_m[,5] <- rep(sr_effects_1_fpv[1], PP)
recov_slope_true_m[,6] <- rep(sr_effects_1_fpv[2], PP)

parameter_recovery_slopes_plots(PP, stanfit_fpv, FPV_test, recov_slope_true_m, "FPV")

parameter_recovery_decay_plots(PP, stanfit_fpv, FPV_test, decay_curve_fpv, "FPV")
parameter_recovery_flow_plots(PP, stanfit_fpv, FPV_test, flow_rate_fpv, "FPV")
