...
parameter_recovery_points_plots <- function(PP, fit, Input,  recov_res_true_m, ID){
 recov_res_latent_m <- recov_res_latent_l <- recov_res_latent_h <- matrix(NA, nrow=PP, ncol=9)
...
# Now organize estimates of interest
 latent_res <- matrix(nrow=dim(sr_sigma)[1], ncol=9)  
 ...
 for( q in 1:dim(sr_sigma)[1]){
  ...
  if("focal_effects" %in%  fit[[k]]@model_pars){
   ...
   latent_res[q,8] <- focal_effects[q,1]
   } 

  if("focal_effects" %in%  fit[[k]]@model_pars){
     target_effects = extract(fit[[k]],pars="target_effects")$target_effects
   latent_res[q,9] <- target_effects[q,1]
   }
...
EstimateOrder <- c("In block", "Out Block", "Dyadic rho",  "Dyadic sigma", 
