...
N_id_sr_rho = 100
N_groups_sr_rho = 3
gprobs_sr_rho = c(0.2, 0.5, 0.3)
in_block_sr_rho = 0.02
out_block_sr_rho = 0.002
sr_mu_sr_rho = c(0,0)  
...
G = simulate_sbm_plus_srm_network(N_id = N_id_sr_rho, 
                                 N_groups = N_groups_sr_rho, 
                                 group_probs = gprobs_sr_rho,
                                 in_block = in_block_sr_rho, 
                                 out_block = out_block_sr_rho,
                                 sr_mu = sr_mu_sr_rho,  
...
model_dat = make_strand_data(self_report=list(G$network),  group_ids=factor(G$group_id), individual_covariates=data.frame(Status=predictor_1_sr_rho), dyadic_covariates=NULL)
