...
N_id_dr_rho = 100
N_groups_dr_rho = 3
gprobs_dr_rho = c(0.2, 0.5, 0.3)
in_block_dr_rho = 0.02
out_block_dr_rho = 0.002
sr_mu_dr_rho = c(0,0)  
...
predictor_1_dr_rho = rbern(100, 0)
sr_effects_1_dr_rho = c(-0.3, 1.3)

G = simulate_sbm_plus_srm_network(N_id = N_id_dr_rho, 
                                 N_groups = N_groups_dr_rho, 
                                 group_probs = gprobs_dr_rho,
                                 in_block = in_block_dr_rho, 
                                 out_block = out_block_dr_rho,
                                 sr_mu = sr_mu_dr_rho,  
...
model_dat = make_strand_data(self_report=list(G$network),  group_ids=factor(G$group_id), individual_covariates=data.frame(Status=predictor_1_dr_rho), dyadic_covariates=NULL)
