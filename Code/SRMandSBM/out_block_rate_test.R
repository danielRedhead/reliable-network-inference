####### FALSE POSITIVE RATE #######
PP = 20
out_block_rate_test = seq(0.0015 , 0.01, length.out=PP)
...
gprobs_out_block = c(0.2, 0.5, 0.3)
in_block_out_block = 0.02
...
G = simulate_sbm_plus_srm_network(N_id = N_id_out_block, 
                                 N_groups = N_groups_out_block, 
                                 group_probs = gprobs_out_block,
...
model_dat = make_strand_data(self_report=list(G$network),  group_ids=factor(G$group_id), individual_covariates=data.frame(Status=predictor_1_out_block), dyadic_covariates=NULL)
